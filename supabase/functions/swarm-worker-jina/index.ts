...
Responde APENAS com JSON válido, SEM markdown fences:
...
Regras: max 5 items, se nada relevante → {"items":[]}, NUNCA inventes, usa PT-PT.`;
  ...
  const cleaned = rawText.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```\s*$/i, "").trim();
  let parsed: unknown;
  try { parsed = JSON.parse(cleaned); }
  catch { throw new Error(`haiku_json_parse: ${cleaned.slice(0, 200)}`); }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Actualizar run — usar colunas existentes + novas B2
    await supabase.from("swarm_runs").update({
      ...
      cost_eur_cents: Math.round(totalCostEur * 100),
      error: lastError,
