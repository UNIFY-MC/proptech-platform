...
2. `.claude/state/agents/coo-agent.md` — o teu estado
3. `.claude/state/triggers.md` secção `## Activos` — se houver linha `TO coo-agent`, trata primeiro

**Depois** de cada trabalho, actualiza `.claude/state/agents/coo-agent.md` no formato:
...
`[YYYY-MM-DDTHH:mmZ] FROM coo-agent → TO <target>: <pedido> [refs]`

**Sem actualizar `agents/coo-agent.md` = trabalho não terminado.** Esta regra é tão importante como a tua função técnica.