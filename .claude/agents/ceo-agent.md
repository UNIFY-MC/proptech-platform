...
2. `.claude/state/agents/ceo-agent.md` — o teu estado
3. `.claude/state/triggers.md` secção `## Activos` — se houver linha `TO ceo-agent`, trata primeiro

**Depois** de cada trabalho, actualiza `.claude/state/agents/ceo-agent.md` no formato:
...
`[YYYY-MM-DDTHH:mmZ] FROM ceo-agent → TO <target>: <pedido> [refs]`

**Sem actualizar `agents/ceo-agent.md` = trabalho não terminado.** Esta regra é tão importante como a tua função técnica.