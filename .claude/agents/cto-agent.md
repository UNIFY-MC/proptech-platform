...
2. `.claude/state/agents/cto-agent.md` — o teu estado
3. `.claude/state/triggers.md` secção `## Activos` — se houver linha `TO cto-agent`, trata primeiro

**Depois** de cada trabalho, actualiza `.claude/state/agents/cto-agent.md` no formato:
...
`[YYYY-MM-DDTHH:mmZ] FROM cto-agent → TO <target>: <pedido> [refs]`

**Sem actualizar `agents/cto-agent.md` = trabalho não terminado.** Esta regra é tão importante como a tua função técnica.