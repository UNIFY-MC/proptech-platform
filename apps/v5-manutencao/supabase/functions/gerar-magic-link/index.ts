 ...
 *
 * Sprint 1D · Day 2 (refactored P1 fix) + Day 5.5 (equipamento_id) · Receipt Trojan Horse Alpha
 ...
 *     equipamento_id?: string (UUID — deve pertencer à org do owner),
 ...
 *   403 — owner sem organização (onboarding incompleto) ou equipamento não pertence à org
 *   404 — equipamento_id não existe
...
function validateInput(raw: unknown): ValidationResult {
  ...
  if (r.localizacao_id !== undefined && r.localizacao_id !== null) {
    if (typeof r.localizacao_id !== "string" || !UUID_REGEX.test(r.localizacao_id)) {
  ...
  if (r.equipamento_id !== undefined && r.equipamento_id !== null) {
    if (typeof r.equipamento_id !== "string" || !UUID_REGEX.test(r.equipamento_id)) {
      return { ok: false, error: "equipamento_id deve ser UUID." };
    }
...
Deno.serve(async (req: Request) => {
  ...
  // ── 4b. Validar equipamento_id (se fornecido) ─────────────────────────────
  if (payload.equipamento_id) {
    ...
    if (eqErr) {
      console.error(JSON.stringify({ event: "erro_select_equipamento", equipamento_id: payload.equipamento_id, error: eqErr.message, ts: new Date().toISOString() }));
      return jsonErr("Erro interno.", 500, "db_equipamento");
    }
    if (!eq) {
      return jsonErr("Equipamento não encontrado.", 404, "equipamento_not_found");
    }

    // Verificar que a localização do equipamento pertence à org do owner
    ...
    if (locErr) {
      console.error(JSON.stringify({ event: "erro_select_localizacao", localizacao_id: eq.localizacao_id, error: locErr.message, ts: new Date().toISOString() }));
      return jsonErr("Erro interno.", 500, "db_localizacao");
    }
    if (!loc) {
      console.log(JSON.stringify({ event: "equipamento_acesso_negado", equipamento_id: payload.equipamento_id, pessoa_id: ownerPessoaId, ts: new Date().toISOString() }));
      return jsonErr("Equipamento não pertence à tua organização.", 403, "equipamento_forbidden");
    }
  }
