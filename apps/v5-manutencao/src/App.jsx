...
import ChatSuporteScreen from './ChatSuporteScreen.jsx'
...
import MissoesScreen from './screens/MissoesScreen.jsx'
import PrestadoresEquipaScreen from './screens/PrestadoresEquipaScreen.jsx'
...
export default function App() {
  ...
  const cliOver  = ['nova','ordem','chat_c','chat_ordem_c','score_detail','alerta_detail','owners_club','chat_prestador',
    ...
    'missoes_semana','prestadores_equipa','poupancas_detalhe',
    ...
    <>
      ...
      {role!=='admin' && <div style={{ maxWidth:430, margin:'0 auto', minHeight:'100vh', background:'#f8fafc', position:'relative', boxShadow:'0 0 80px rgba(0,0,0,0.5)' }}>
        ...
        {role==='cliente' && <>
          ...
          {ecra==='notificacoes'       && <NotificacoesScreen onBack={()=>setEcra('home')} onNavigateAlerta={()=>setEcra('alerta_detail')} />}
          {ecra==='chat_suporte'       && <ChatSuporteScreen onBack={()=>setEcra('home')} />}
          ...
          {ecra==='missoes_semana'      && <MissoesScreen onBack={()=>setEcra('home')} onNavigateAction={(a)=>{ setEcra(a==='moradas'?'moradas_screen':a==='adicionar_energia'?'adicionar_energia':a==='adicionar_camara'?'adicionar_camara':'adicionar_doc') }} />}
          ...
          {ecra==='servicos_lista'     && <ServicosListaScreen filtro={selNav?.servicosListaFiltro||'populares'} titulo={selNav?.servicosListaTitulo||'Mais contratados'} onBack={()=>setEcra('home')} onNavigateServico={(s)=>{ setSelNav(p=>({...p,servico:s,backFrom:'servicos_lista'})); setEcra('servico_detail') }} />}
          {ecra==='orcamentos_landing'  && <OrcamentosLandingScreen onBack={()=>setEcra('home')} onIniciar={()=>setEcra('orcamento_wizard')} />}
          {ecra==='orcamento_wizard'    && <OrcamentoWizardScreen onBack={()=>{ const wasSkip=selNav?.wizardSkipStep1; const backEcra=selNav?.wizardBackTarget||(wasSkip?'categoria_detail':'orcamentos_landing'); setSelNav(p=>({...p,wizardSkipStep1:null,wizardCategoria:null,wizardDescricao:null,wizardBackTarget:null})); setEcra(backEcra) }} onConfirmado={(newId)=>{ if(newId) setSelNav(p=>({...p,orcamentoId:newId})); setEcra('orcamento_confirmado') }} localizacaoId={imovelAtivo?.id} categoriaSlug={selNav?.wizardCategoria} skipStep1={!!selNav?.wizardSkipStep1} descricaoInicial={selNav?.wizardDescricao} />}
            ...
            {!cliOver && <>
              <PerfilSheet
                ...
                onNavigate={(target)=>{
                  ...
                  else if(target==='wishlist')     { setShowPerfilSheet(false); setPrevTab(tab); setTab('perfil'); setEcra('wishlist') }
              ...
              {tab==='perfil'   && ecra==='wishlist' && (
                <CWishlist
                  authUser={authUser}
                  onBack={()=>{ setTab(prevTab || 'inicio'); setEcra('home') }}
              ...
              <PerfilDrawer
                ...
                onNavigate={(target)=>{
                  ...
                  else if(target==='wishlist')      { close(); setPrevTab(tab); setTab('perfil'); setEcra('wishlist') }
