    ...
    <>
      ...
      {role!=='admin' && <div style={{ maxWidth:430, margin:'0 auto', minHeight:'100vh', background:'#f8fafc', position:'relative', boxShadow:'0 0 80px rgba(0,0,0,0.5)' }}>
        ...
        {role==='cliente' && <>
          ...
          {ecra==='orcamento_wizard'    && <OrcamentoWizardScreen onBack={()=>{ const wasSkip=selNav?.wizardSkipStep1; setSelNav(p=>({...p,wizardSkipStep1:null,wizardCategoria:null})); setEcra(wasSkip ? 'categoria_detail' : 'orcamentos_landing') }} onConfirmado={(newId)=>{ if(newId) setSelNav(p=>({...p,orcamentoId:newId})); setEcra('orcamento_confirmado') }} localizacaoId={imovelAtivo?.id} categoriaSlug={selNav?.wizardCategoria} skipStep1={!!selNav?.wizardSkipStep1} />}
