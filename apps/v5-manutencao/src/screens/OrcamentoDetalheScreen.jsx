...
import { MOCK_ORCAMENTOS_PEDIDOS, MOCK_PROPOSTAS_ORCAMENTO } from '../data/mock.js'
...
export default function OrcamentoDetalheScreen({ orcamentoId, onBack }) {
  ...
  const orc       = MOCK_ORCAMENTOS_PEDIDOS.find(o => o.id === orcamentoId) || MOCK_ORCAMENTOS_PEDIDOS[0]
  const propostas = MOCK_PROPOSTAS_ORCAMENTO.filter(p => p.orcamento_id === (orc?.id))
  const progresso = orc ? Math.round((orc.propostas_recebidas / orc.propostas_alvo) * 100) : 0
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, paddingBottom:40 }}>
      ...
      <div style={{ background:`linear-gradient(145deg,${G},${GM})`, padding:'14px 16px 20px', color:'#fff' }}>
        ...
        <div style={{ fontSize:20, fontWeight:700, fontFamily:'Georgia,serif', lineHeight:1.2, marginBottom:6 }}>{orc.titulo}</div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'rgba(255,255,255,.75)' }}>
          <span>{orc.propostas_recebidas} de {orc.propostas_alvo} propostas recebidas</span>
          <span>{orc.prazo_resposta ? tempoRestante(orc.prazo_resposta) : ''}</span>
        </div>
      </div>
      ...
      <div style={{ padding:'12px 14px 0' }}>
        ...
        {/* ── Fotos ── */}
        <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, padding:'13px 14px', marginBottom:10 }}>
          <div style={{ fontSize:12, fontWeight:700, color:C.ink, marginBottom:8 }}>📸 Fotos ({orc.fotos_count})</div>
          <div style={{ display:'flex', gap:8 }}>
            {[...Array(orc.fotos_count)].map((_, i) => (
          ...
          <div key={prop.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderLeft:`4px solid ${G}`, borderRadius:12, padding:'13px 14px', marginBottom:9 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
              <div style={{ width:36, height:36, borderRadius:'50%', background:`linear-gradient(135deg,${G},${GM})`, color:'#fff', fontWeight:700, fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                {prop.prestador_iniciais}
              </div>
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ fontSize:12, fontWeight:700, color:C.ink }}>{prop.prestador_nome}</span>
                  {prop.verificado && <span style={{ fontSize:9, background:C.greenXl, color:G, padding:'2px 6px', borderRadius:4, fontWeight:700 }}>✓ Verificado</span>}
                </div>
                <div style={{ fontSize:10, color:'#F59E0B' }}>{'★'.repeat(Math.round(prop.rating))} <span style={{ color:C.slate }}>{prop.rating}</span></div>
              </div>
              <div style={{ textAlign:'right', flexShrink:0 }}>
                <div style={{ fontSize:18, fontWeight:800, color:G }}>{prop.preco.toLocaleString('pt-PT')}€</div>
                <div style={{ fontSize:10, color:C.slate }}>{prop.tempo_dias} dias úteis</div>
              </div>
            </div>
        ...
        <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, padding:'13px 14px', marginBottom:10 }}>
          ...
          {[
            { ic:'✅', l:'Pedido enviado', t: new Date(orc.enviado_em).toLocaleDateString('pt-PT', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }), done:true },
            { ic:'📬', l:`${orc.propostas_recebidas} proposta${orc.propostas_recebidas !== 1 ? 's' : ''} recebida${orc.propostas_recebidas !== 1 ? 's' : ''}`, t:'Em progresso', done: orc.propostas_recebidas > 0 },
            { ic:'🤝', l:'Aceitar proposta', t:'Pendente', done:false },
            { ic:'🏠', l:'Trabalho concluído', t:'Pendente', done:false },
          ].map((step, i) => (
